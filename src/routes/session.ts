...
function validateUserIdentity(identity: unknown): { valid: boolean; error?: string } {
    ...
    if (trimmed.length > MAX_IDENTITY_LENGTH) {
        return {
            valid: false,
            error: `userIdentity must be ${MAX_IDENTITY_LENGTH} characters or less`,
        };
    }
    ...
    if (!VALID_IDENTIFIER_REGEX.test(trimmed)) {
        return {
            valid: false,
            error: 'userIdentity can only contain letters, numbers, underscores, and hyphens',
        };
    }
...
function validateRoomName(roomName: unknown): { valid: boolean; error?: string } {
    ...
    if (trimmed.length > MAX_ROOM_NAME_LENGTH) {
        return {
            valid: false,
            error: `roomName must be ${MAX_ROOM_NAME_LENGTH} characters or less`,
        };
    }
    ...
    if (!VALID_IDENTIFIER_REGEX.test(trimmed)) {
        return {
            valid: false,
            error: 'roomName can only contain letters, numbers, underscores, and hyphens',
        };
    }
...
function validateAvatarConfig(agentConfig: AgentConfig | undefined): {
    valid: boolean;
    error?: string;
} {
    ...
    if (!avatarId || typeof avatarId !== 'string' || avatarId.trim().length === 0) {
        return {
            valid: false,
            error: 'avatar.anam.avatarId is required when avatar.enabled is true',
        };
    }
