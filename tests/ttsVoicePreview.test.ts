...
describe('GET /v1/config/tts-voice-preview', () => {
    it('returns 400 for invalid query', async () => {
        const app = await importFreshApp();
        const res = await request(app).get('/v1/config/tts-voice-preview');
    ...
    it('returns 400 for disallowed host', async () => {
        ...
        const res = await request(app).get('/v1/config/tts-voice-preview').query({
            providerId: 'cartesia',
